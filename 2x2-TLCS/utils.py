...
def import_train_configuration(config_file):
    ...
    config['network'] = content['dir']['network']
    config['route'] = content['dir']['route']
    return config
...
def set_sumo(gui, sumocfg_file_name, max_steps, path):
    """
    ...
    sumo_cmd = [sumoBinary, "-c", os.path.join('network', sumocfg_file_name), "--no-step-log", "true", "--waiting-time-memory", str(max_steps),
                '--summary', f'{path}Summary.xml', '--queue-output', f'{path}QueueInfo.xml', f'--tripinfo-output', '{path}VehicleInfo.xml'
                ]
